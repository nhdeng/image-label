import './style.css';
import jA, { forwardRef as nr, useState as UA, useRef as qA, useEffect as Ir, useImperativeHandle as hr } from "react";
var CA = {}, wr = {
  get exports() {
    return CA;
  },
  set exports(R) {
    CA = R;
  }
}, P = {};
/**
 * @license React
 * react-jsx-runtime.production.min.js
 *
 * Copyright (c) Facebook, Inc. and its affiliates.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */
var JA;
function ur() {
  if (JA)
    return P;
  JA = 1;
  var R = jA, v = Symbol.for("react.element"), O = Symbol.for("react.fragment"), n = Object.prototype.hasOwnProperty, b = R.__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED.ReactCurrentOwner, y = { key: !0, ref: !0, __self: !0, __source: !0 };
  function U(G, D, q) {
    var S, x = {}, t = null, J = null;
    q !== void 0 && (t = "" + q), D.key !== void 0 && (t = "" + D.key), D.ref !== void 0 && (J = D.ref);
    for (S in D)
      n.call(D, S) && !y.hasOwnProperty(S) && (x[S] = D[S]);
    if (G && G.defaultProps)
      for (S in D = G.defaultProps, D)
        x[S] === void 0 && (x[S] = D[S]);
    return { $$typeof: v, type: G, key: t, ref: J, props: x, _owner: b.current };
  }
  return P.Fragment = O, P.jsx = U, P.jsxs = U, P;
}
var N = {};
/**
 * @license React
 * react-jsx-runtime.development.js
 *
 * Copyright (c) Facebook, Inc. and its affiliates.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */
var WA;
function Mr() {
  return WA || (WA = 1, process.env.NODE_ENV !== "production" && function() {
    var R = jA, v = Symbol.for("react.element"), O = Symbol.for("react.portal"), n = Symbol.for("react.fragment"), b = Symbol.for("react.strict_mode"), y = Symbol.for("react.profiler"), U = Symbol.for("react.provider"), G = Symbol.for("react.context"), D = Symbol.for("react.forward_ref"), q = Symbol.for("react.suspense"), S = Symbol.for("react.suspense_list"), x = Symbol.for("react.memo"), t = Symbol.for("react.lazy"), J = Symbol.for("react.offscreen"), H = Symbol.iterator, _ = "@@iterator";
    function gA(A) {
      if (A === null || typeof A != "object")
        return null;
      var r = H && A[H] || A[_];
      return typeof r == "function" ? r : null;
    }
    var g = R.__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED;
    function F(A) {
      {
        for (var r = arguments.length, e = new Array(r > 1 ? r - 1 : 0), C = 1; C < r; C++)
          e[C - 1] = arguments[C];
        EA("error", A, e);
      }
    }
    function EA(A, r, e) {
      {
        var C = g.ReactDebugCurrentFrame, i = C.getStackAddendum();
        i !== "" && (r += "%s", e = e.concat([i]));
        var c = e.map(function(B) {
          return String(B);
        });
        c.unshift("Warning: " + r), Function.prototype.apply.call(console[A], console, c);
      }
    }
    var BA = !1, QA = !1, sA = !1, T = !1, s = !1, o;
    o = Symbol.for("react.module.reference");
    function h(A) {
      return !!(typeof A == "string" || typeof A == "function" || A === n || A === y || s || A === b || A === q || A === S || T || A === J || BA || QA || sA || typeof A == "object" && A !== null && (A.$$typeof === t || A.$$typeof === x || A.$$typeof === U || A.$$typeof === G || A.$$typeof === D || // This needs to include all possible module reference object
      // types supported by any Flight configuration anywhere since
      // we don't know which Flight build this will end up being used
      // with.
      A.$$typeof === o || A.getModuleId !== void 0));
    }
    function u(A, r, e) {
      var C = A.displayName;
      if (C)
        return C;
      var i = r.displayName || r.name || "";
      return i !== "" ? e + "(" + i + ")" : e;
    }
    function I(A) {
      return A.displayName || "Context";
    }
    function a(A) {
      if (A == null)
        return null;
      if (typeof A.tag == "number" && F("Received an unexpected object in getComponentNameFromType(). This is likely a bug in React. Please file an issue."), typeof A == "function")
        return A.displayName || A.name || null;
      if (typeof A == "string")
        return A;
      switch (A) {
        case n:
          return "Fragment";
        case O:
          return "Portal";
        case y:
          return "Profiler";
        case b:
          return "StrictMode";
        case q:
          return "Suspense";
        case S:
          return "SuspenseList";
      }
      if (typeof A == "object")
        switch (A.$$typeof) {
          case G:
            var r = A;
            return I(r) + ".Consumer";
          case U:
            var e = A;
            return I(e._context) + ".Provider";
          case D:
            return u(A, A.render, "ForwardRef");
          case x:
            var C = A.displayName || null;
            return C !== null ? C : a(A.type) || "Memo";
          case t: {
            var i = A, c = i._payload, B = i._init;
            try {
              return a(B(c));
            } catch {
              return null;
            }
          }
        }
      return null;
    }
    var K = Object.assign, d = 0, L, j, W, oA, DA, lA, FA;
    function fA() {
    }
    fA.__reactDisabledLog = !0;
    function mA() {
      {
        if (d === 0) {
          L = console.log, j = console.info, W = console.warn, oA = console.error, DA = console.group, lA = console.groupCollapsed, FA = console.groupEnd;
          var A = {
            configurable: !0,
            enumerable: !0,
            value: fA,
            writable: !0
          };
          Object.defineProperties(console, {
            info: A,
            log: A,
            warn: A,
            error: A,
            group: A,
            groupCollapsed: A,
            groupEnd: A
          });
        }
        d++;
      }
    }
    function zA() {
      {
        if (d--, d === 0) {
          var A = {
            configurable: !0,
            enumerable: !0,
            writable: !0
          };
          Object.defineProperties(console, {
            log: K({}, A, {
              value: L
            }),
            info: K({}, A, {
              value: j
            }),
            warn: K({}, A, {
              value: W
            }),
            error: K({}, A, {
              value: oA
            }),
            group: K({}, A, {
              value: DA
            }),
            groupCollapsed: K({}, A, {
              value: lA
            }),
            groupEnd: K({}, A, {
              value: FA
            })
          });
        }
        d < 0 && F("disabledDepth fell below zero. This is a bug in React. Please file an issue.");
      }
    }
    var iA = g.ReactCurrentDispatcher, aA;
    function $(A, r, e) {
      {
        if (aA === void 0)
          try {
            throw Error();
          } catch (i) {
            var C = i.stack.trim().match(/\n( *(at )?)/);
            aA = C && C[1] || "";
          }
        return `
` + aA + A;
      }
    }
    var cA = !1, AA;
    {
      var HA = typeof WeakMap == "function" ? WeakMap : Map;
      AA = new HA();
    }
    function SA(A, r) {
      if (!A || cA)
        return "";
      {
        var e = AA.get(A);
        if (e !== void 0)
          return e;
      }
      var C;
      cA = !0;
      var i = Error.prepareStackTrace;
      Error.prepareStackTrace = void 0;
      var c;
      c = iA.current, iA.current = null, mA();
      try {
        if (r) {
          var B = function() {
            throw Error();
          };
          if (Object.defineProperty(B.prototype, "props", {
            set: function() {
              throw Error();
            }
          }), typeof Reflect == "object" && Reflect.construct) {
            try {
              Reflect.construct(B, []);
            } catch (k) {
              C = k;
            }
            Reflect.construct(A, [], B);
          } else {
            try {
              B.call();
            } catch (k) {
              C = k;
            }
            A.call(B.prototype);
          }
        } else {
          try {
            throw Error();
          } catch (k) {
            C = k;
          }
          A();
        }
      } catch (k) {
        if (k && C && typeof k.stack == "string") {
          for (var E = k.stack.split(`
`), Y = C.stack.split(`
`), w = E.length - 1, M = Y.length - 1; w >= 1 && M >= 0 && E[w] !== Y[M]; )
            M--;
          for (; w >= 1 && M >= 0; w--, M--)
            if (E[w] !== Y[M]) {
              if (w !== 1 || M !== 1)
                do
                  if (w--, M--, M < 0 || E[w] !== Y[M]) {
                    var p = `
` + E[w].replace(" at new ", " at ");
                    return A.displayName && p.includes("<anonymous>") && (p = p.replace("<anonymous>", A.displayName)), typeof A == "function" && AA.set(A, p), p;
                  }
                while (w >= 1 && M >= 0);
              break;
            }
        }
      } finally {
        cA = !1, iA.current = c, zA(), Error.prepareStackTrace = i;
      }
      var z = A ? A.displayName || A.name : "", bA = z ? $(z) : "";
      return typeof A == "function" && AA.set(A, bA), bA;
    }
    function TA(A, r, e) {
      return SA(A, !1);
    }
    function XA(A) {
      var r = A.prototype;
      return !!(r && r.isReactComponent);
    }
    function rA(A, r, e) {
      if (A == null)
        return "";
      if (typeof A == "function")
        return SA(A, XA(A));
      if (typeof A == "string")
        return $(A);
      switch (A) {
        case q:
          return $("Suspense");
        case S:
          return $("SuspenseList");
      }
      if (typeof A == "object")
        switch (A.$$typeof) {
          case D:
            return TA(A.render);
          case x:
            return rA(A.type, r, e);
          case t: {
            var C = A, i = C._payload, c = C._init;
            try {
              return rA(c(i), r, e);
            } catch {
            }
          }
        }
      return "";
    }
    var eA = Object.prototype.hasOwnProperty, KA = {}, YA = g.ReactDebugCurrentFrame;
    function tA(A) {
      if (A) {
        var r = A._owner, e = rA(A.type, A._source, r ? r.type : null);
        YA.setExtraStackFrame(e);
      } else
        YA.setExtraStackFrame(null);
    }
    function PA(A, r, e, C, i) {
      {
        var c = Function.call.bind(eA);
        for (var B in A)
          if (c(A, B)) {
            var E = void 0;
            try {
              if (typeof A[B] != "function") {
                var Y = Error((C || "React class") + ": " + e + " type `" + B + "` is invalid; it must be a function, usually from the `prop-types` package, but received `" + typeof A[B] + "`.This often happens because of typos such as `PropTypes.function` instead of `PropTypes.func`.");
                throw Y.name = "Invariant Violation", Y;
              }
              E = A[B](r, B, C, e, null, "SECRET_DO_NOT_PASS_THIS_OR_YOU_WILL_BE_FIRED");
            } catch (w) {
              E = w;
            }
            E && !(E instanceof Error) && (tA(i), F("%s: type specification of %s `%s` is invalid; the type checker function must return `null` or an `Error` but returned a %s. You may have forgotten to pass an argument to the type checker creator (arrayOf, instanceOf, objectOf, oneOf, oneOfType, and shape all require an argument).", C || "React class", e, B, typeof E), tA(null)), E instanceof Error && !(E.message in KA) && (KA[E.message] = !0, tA(i), F("Failed %s type: %s", e, E.message), tA(null));
          }
      }
    }
    var NA = Array.isArray;
    function nA(A) {
      return NA(A);
    }
    function ZA(A) {
      {
        var r = typeof Symbol == "function" && Symbol.toStringTag, e = r && A[Symbol.toStringTag] || A.constructor.name || "Object";
        return e;
      }
    }
    function VA(A) {
      try {
        return LA(A), !1;
      } catch {
        return !0;
      }
    }
    function LA(A) {
      return "" + A;
    }
    function yA(A) {
      if (VA(A))
        return F("The provided key is an unsupported type %s. This value must be coerced to a string before before using it here.", ZA(A)), LA(A);
    }
    var X = g.ReactCurrentOwner, _A = {
      key: !0,
      ref: !0,
      __self: !0,
      __source: !0
    }, vA, xA, IA;
    IA = {};
    function $A(A) {
      if (eA.call(A, "ref")) {
        var r = Object.getOwnPropertyDescriptor(A, "ref").get;
        if (r && r.isReactWarning)
          return !1;
      }
      return A.ref !== void 0;
    }
    function Ar(A) {
      if (eA.call(A, "key")) {
        var r = Object.getOwnPropertyDescriptor(A, "key").get;
        if (r && r.isReactWarning)
          return !1;
      }
      return A.key !== void 0;
    }
    function rr(A, r) {
      if (typeof A.ref == "string" && X.current && r && X.current.stateNode !== r) {
        var e = a(X.current.type);
        IA[e] || (F('Component "%s" contains the string ref "%s". Support for string refs will be removed in a future major release. This case cannot be automatically converted to an arrow function. We ask you to manually fix this case by using useRef() or createRef() instead. Learn more about using refs safely here: https://reactjs.org/link/strict-mode-string-ref', a(X.current.type), A.ref), IA[e] = !0);
      }
    }
    function er(A, r) {
      {
        var e = function() {
          vA || (vA = !0, F("%s: `key` is not a prop. Trying to access it will result in `undefined` being returned. If you need to access the same value within the child component, you should pass it as a different prop. (https://reactjs.org/link/special-props)", r));
        };
        e.isReactWarning = !0, Object.defineProperty(A, "key", {
          get: e,
          configurable: !0
        });
      }
    }
    function tr(A, r) {
      {
        var e = function() {
          xA || (xA = !0, F("%s: `ref` is not a prop. Trying to access it will result in `undefined` being returned. If you need to access the same value within the child component, you should pass it as a different prop. (https://reactjs.org/link/special-props)", r));
        };
        e.isReactWarning = !0, Object.defineProperty(A, "ref", {
          get: e,
          configurable: !0
        });
      }
    }
    var Cr = function(A, r, e, C, i, c, B) {
      var E = {
        // This tag allows us to uniquely identify this as a React Element
        $$typeof: v,
        // Built-in properties that belong on the element
        type: A,
        key: r,
        ref: e,
        props: B,
        // Record the component responsible for creating this element.
        _owner: c
      };
      return E._store = {}, Object.defineProperty(E._store, "validated", {
        configurable: !1,
        enumerable: !1,
        writable: !0,
        value: !1
      }), Object.defineProperty(E, "_self", {
        configurable: !1,
        enumerable: !1,
        writable: !1,
        value: C
      }), Object.defineProperty(E, "_source", {
        configurable: !1,
        enumerable: !1,
        writable: !1,
        value: i
      }), Object.freeze && (Object.freeze(E.props), Object.freeze(E)), E;
    };
    function gr(A, r, e, C, i) {
      {
        var c, B = {}, E = null, Y = null;
        e !== void 0 && (yA(e), E = "" + e), Ar(r) && (yA(r.key), E = "" + r.key), $A(r) && (Y = r.ref, rr(r, i));
        for (c in r)
          eA.call(r, c) && !_A.hasOwnProperty(c) && (B[c] = r[c]);
        if (A && A.defaultProps) {
          var w = A.defaultProps;
          for (c in w)
            B[c] === void 0 && (B[c] = w[c]);
        }
        if (E || Y) {
          var M = typeof A == "function" ? A.displayName || A.name || "Unknown" : A;
          E && er(B, M), Y && tr(B, M);
        }
        return Cr(A, E, Y, i, C, X.current, B);
      }
    }
    var hA = g.ReactCurrentOwner, pA = g.ReactDebugCurrentFrame;
    function m(A) {
      if (A) {
        var r = A._owner, e = rA(A.type, A._source, r ? r.type : null);
        pA.setExtraStackFrame(e);
      } else
        pA.setExtraStackFrame(null);
    }
    var wA;
    wA = !1;
    function uA(A) {
      return typeof A == "object" && A !== null && A.$$typeof === v;
    }
    function RA() {
      {
        if (hA.current) {
          var A = a(hA.current.type);
          if (A)
            return `

Check the render method of \`` + A + "`.";
        }
        return "";
      }
    }
    function Er(A) {
      {
        if (A !== void 0) {
          var r = A.fileName.replace(/^.*[\\\/]/, ""), e = A.lineNumber;
          return `

Check your code at ` + r + ":" + e + ".";
        }
        return "";
      }
    }
    var OA = {};
    function Br(A) {
      {
        var r = RA();
        if (!r) {
          var e = typeof A == "string" ? A : A.displayName || A.name;
          e && (r = `

Check the top-level render call using <` + e + ">.");
        }
        return r;
      }
    }
    function GA(A, r) {
      {
        if (!A._store || A._store.validated || A.key != null)
          return;
        A._store.validated = !0;
        var e = Br(r);
        if (OA[e])
          return;
        OA[e] = !0;
        var C = "";
        A && A._owner && A._owner !== hA.current && (C = " It was passed a child from " + a(A._owner.type) + "."), m(A), F('Each child in a list should have a unique "key" prop.%s%s See https://reactjs.org/link/warning-keys for more information.', e, C), m(null);
      }
    }
    function dA(A, r) {
      {
        if (typeof A != "object")
          return;
        if (nA(A))
          for (var e = 0; e < A.length; e++) {
            var C = A[e];
            uA(C) && GA(C, r);
          }
        else if (uA(A))
          A._store && (A._store.validated = !0);
        else if (A) {
          var i = gA(A);
          if (typeof i == "function" && i !== A.entries)
            for (var c = i.call(A), B; !(B = c.next()).done; )
              uA(B.value) && GA(B.value, r);
        }
      }
    }
    function Qr(A) {
      {
        var r = A.type;
        if (r == null || typeof r == "string")
          return;
        var e;
        if (typeof r == "function")
          e = r.propTypes;
        else if (typeof r == "object" && (r.$$typeof === D || // Note: Memo only checks outer props here.
        // Inner props are checked in the reconciler.
        r.$$typeof === x))
          e = r.propTypes;
        else
          return;
        if (e) {
          var C = a(r);
          PA(e, A.props, "prop", C, A);
        } else if (r.PropTypes !== void 0 && !wA) {
          wA = !0;
          var i = a(r);
          F("Component %s declared `PropTypes` instead of `propTypes`. Did you misspell the property assignment?", i || "Unknown");
        }
        typeof r.getDefaultProps == "function" && !r.getDefaultProps.isReactClassApproved && F("getDefaultProps is only used on classic React.createClass definitions. Use a static property named `defaultProps` instead.");
      }
    }
    function sr(A) {
      {
        for (var r = Object.keys(A.props), e = 0; e < r.length; e++) {
          var C = r[e];
          if (C !== "children" && C !== "key") {
            m(A), F("Invalid prop `%s` supplied to `React.Fragment`. React.Fragment can only have `key` and `children` props.", C), m(null);
            break;
          }
        }
        A.ref !== null && (m(A), F("Invalid attribute `ref` supplied to `React.Fragment`."), m(null));
      }
    }
    function kA(A, r, e, C, i, c) {
      {
        var B = h(A);
        if (!B) {
          var E = "";
          (A === void 0 || typeof A == "object" && A !== null && Object.keys(A).length === 0) && (E += " You likely forgot to export your component from the file it's defined in, or you might have mixed up default and named imports.");
          var Y = Er(i);
          Y ? E += Y : E += RA();
          var w;
          A === null ? w = "null" : nA(A) ? w = "array" : A !== void 0 && A.$$typeof === v ? (w = "<" + (a(A.type) || "Unknown") + " />", E = " Did you accidentally export a JSX literal instead of a component?") : w = typeof A, F("React.jsx: type is invalid -- expected a string (for built-in components) or a class/function (for composite components) but got: %s.%s", w, E);
        }
        var M = gr(A, r, e, i, c);
        if (M == null)
          return M;
        if (B) {
          var p = r.children;
          if (p !== void 0)
            if (C)
              if (nA(p)) {
                for (var z = 0; z < p.length; z++)
                  dA(p[z], A);
                Object.freeze && Object.freeze(p);
              } else
                F("React.jsx: Static children should always be an array. You are likely explicitly calling React.jsxs or React.jsxDEV. Use the Babel transform instead.");
            else
              dA(p, A);
        }
        return A === n ? sr(M) : Qr(M), M;
      }
    }
    function or(A, r, e) {
      return kA(A, r, e, !0);
    }
    function ir(A, r, e) {
      return kA(A, r, e, !1);
    }
    var ar = ir, cr = or;
    N.Fragment = n, N.jsx = ar, N.jsxs = cr;
  }()), N;
}
(function(R) {
  process.env.NODE_ENV === "production" ? R.exports = ur() : R.exports = Mr();
})(wr);
const MA = CA.jsx, Dr = CA.jsxs;
const lr = 1e3, Fr = 800;
let l = [], Z = 1, f = {}, Q = {}, V = -1;
const Sr = nr(
  ({
    url: R,
    lineProps: v,
    containerProps: O,
    pointProps: n,
    getPoints: b,
    fullScreen: y = !1
  }, U) => {
    const [G, D] = UA(!1), [q, S] = UA(!1), x = qA(null), t = qA(null);
    Ir(() => {
      var s, o, h;
      return (s = t.current) == null || s.addEventListener("wheel", _), (o = t.current) == null || o.addEventListener("mousedown", J), (h = t.current) == null || h.addEventListener("mousemove", H), () => {
        var u, I, a;
        f.clearRect(0, 0, 1e5, 1e5), l = [], (u = t.current) == null || u.removeEventListener("wheel", _), (I = t.current) == null || I.removeEventListener("mousedown", J), (a = t.current) == null || a.removeEventListener("mousemove", H);
      };
    }, []);
    const J = (s) => {
      if (!t.current)
        return;
      const { clientX: o, clientY: h, offsetX: u, offsetY: I } = s, a = o - t.current.offsetLeft || 0, K = h - t.current.offsetTop || 0, d = n != null && n.size ? (n == null ? void 0 : n.size) * 2 : 20;
      l.forEach((L, j) => {
        const W = { x: L.x * g(), y: L.y * g() };
        Math.abs(u - W.x) <= d && Math.abs(I - W.y) <= d && (V = j);
      }), V >= 0 ? t.current.onmousemove = function(L) {
        l[V].x = L.offsetX / g(), l[V].y = L.offsetY / g(), f.clearRect(
          0,
          0,
          Q.clientWidth * g(),
          Q.clientHeight * g()
        ), f.drawImage(
          Q,
          0,
          0,
          Q.clientWidth * g(),
          Q.clientHeight * g()
        ), T(g());
      } : document.onmousemove = function(L) {
        if (!t.current)
          return;
        const j = L.clientX - a, W = L.clientY - K;
        t.current.style.left = j + "px", t.current.style.top = W + "px";
      }, document.onmouseup = function() {
        var L;
        t.current && ((L = t.current) == null || L.setAttribute("class", ""), document.onmousemove = null, t.current.onmousemove = null, document.onmouseup = null, V = -1);
      };
    }, H = (s) => {
      if (!t.current)
        return;
      const { offsetX: o, offsetY: h } = s;
      let u = !1;
      const I = n != null && n.size ? (n == null ? void 0 : n.size) * 2 : 20;
      l.forEach((a) => {
        const K = { x: a.x * g(), y: a.y * g() };
        Math.abs(o - K.x) <= I && Math.abs(h - K.y) <= I && (u = !0);
      }), setTimeout(() => {
        var a;
        (a = t.current) == null || a.setAttribute("class", u ? "selected" : "");
      }, 100);
    }, _ = (s) => {
      s.preventDefault(), gA(s), T(g());
    }, gA = (s) => {
      var K;
      let o = 0, h = 0;
      const { deltaY: u, offsetX: I, offsetY: a } = s;
      u < 0 && g() < 5 && (t != null && t.current) && (Q.style.scale = JSON.stringify(g() + 0.1), h = ((K = t.current) == null ? void 0 : K.offsetLeft) - (I * g() / (g() - 0.1) - I), o = t.current.offsetTop - (a * g() / (g() - 0.1) - a), t.current.style.left = h + "px", t.current.style.top = o + "px", t.current.style.width = Q.clientWidth * g() + "px", t.current.style.height = Q.clientHeight * g() + "px", t.current.width = Q.clientWidth * g(), t.current.height = Q.clientHeight * g()), u >= 0 && g() - 0.1 >= 0.1 && (t != null && t.current) && (Q.style.scale = JSON.stringify(g() - 0.1), h = t.current.offsetLeft - I * g() / (g() + 0.1) + I, o = t.current.offsetTop - a * g() / (g() + 0.1) + a, t.current.style.left = h + "px", t.current.style.top = o + "px", t.current.style.width = Q.clientWidth * g() + "px", t.current.style.height = Q.clientHeight * g() + "px", t.current.width = Q.clientWidth * g(), t.current.height = Q.clientHeight * g()), f.drawImage(
        Q,
        0,
        0,
        Q.clientWidth * g(),
        Q.clientHeight * g()
      );
    }, g = () => Number(Q.style.scale), F = () => {
      var s;
      f = (s = t == null ? void 0 : t.current) == null ? void 0 : s.getContext("2d"), D(!0), Q = document.createElement("img"), Q.setAttribute("alt", "trace img"), Q.setAttribute("id", "img"), Q.setAttribute("src", R), Q.onload = function() {
        var a;
        D(!1), (a = x.current) == null || a.append(Q);
        const { width: o, height: h } = Q;
        o > h ? Q.style.width = "80%" : Q.style.height = "80%";
        const { clientWidth: u, clientHeight: I } = Q;
        Z = u / o, t.current && (t.current.style.width = u + "px", t.current.style.height = I + "px", t.current.width = u, t.current.height = I, t.current.style.left = `calc(50% - ${u / 2}px)`, t.current.style.top = `calc(50% - ${I / 2}px)`), f.drawImage(Q, 0, 0, u, I), Q.style.scale = "1", BA(), T(1);
      }, Q.onerror = function() {
        D(!1), S(!0);
      };
    }, EA = () => {
      l = [], F();
    }, BA = () => {
      if (l.length > 0)
        return;
      const { clientWidth: s, clientHeight: o } = Q, h = {
        x: s * (y ? 0 : 0.2),
        y: o * (y ? 0 : 0.2)
      }, u = {
        x: s * (y ? 1 : 0.8),
        y: o * (y ? 0 : 0.2)
      }, I = {
        x: s * (y ? 1 : 0.8),
        y: o * (y ? 1 : 0.8)
      }, a = {
        x: s * (y ? 0 : 0.2),
        y: o * (y ? 1 : 0.8)
      };
      l.push(h), l.push(u), l.push(I), l.push(a);
    }, QA = () => l.length ? l.map(({ x: s, y: o }) => ({
      x: Number((s / Z).toFixed(0)),
      y: Number((o / Z).toFixed(0))
    })) : (console.error("画布暂时没有初始化"), []), sA = (s) => {
      l = s.map(({ x: o, y: h }) => ({
        x: o * Z,
        y: h * Z
      })), F();
    }, T = (s) => {
      l.length && (l.forEach((o) => {
        f.beginPath(), f.arc(
          o.x * s,
          o.y * s,
          (n == null ? void 0 : n.size) || 10,
          0,
          2 * Math.PI
        ), f.fillStyle = (n == null ? void 0 : n.color) || "red", f.fill(), f.closePath();
      }), f.beginPath(), f.setLineDash([10, 5]), f.lineWidth = (v == null ? void 0 : v.width) || 2, f.strokeStyle = (v == null ? void 0 : v.color) || "red", l.forEach(({ x: o, y: h }) => {
        f.lineTo(o * s, h * s);
      }), f.lineTo(l[0].x * s, l[0].y * s), f.stroke(), b && b(QA()));
    };
    return hr(U, () => ({
      onReset: EA,
      setPoints: sA
    })), /* @__PURE__ */ Dr(
      "div",
      {
        ref: x,
        className: "label-image__container",
        style: {
          width: `${(O == null ? void 0 : O.width) || lr}px`,
          height: `${(O == null ? void 0 : O.height) || Fr}px`
        },
        children: [
          G && /* @__PURE__ */ MA("div", { className: "status", children: /* @__PURE__ */ MA(
            "img",
            {
              className: "image-label_loading",
              width: 100,
              height: 100,
              alt: "loading icon",
              src: "data:image/gif;base64,"
            }
          ) }),
          /* @__PURE__ */ MA("canvas", { ref: t })
        ]
      }
    );
  }
);
export {
  Sr as default
};
