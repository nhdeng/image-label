import './style.css';
import jA, { forwardRef as cr, useState as UA, useRef as qA, useEffect as Ir, useImperativeHandle as hr } from "react";
var eA = {}, wr = {
  get exports() {
    return eA;
  },
  set exports(R) {
    eA = R;
  }
}, P = {};
/**
 * @license React
 * react-jsx-runtime.production.min.js
 *
 * Copyright (c) Facebook, Inc. and its affiliates.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */
var JA;
function ur() {
  if (JA)
    return P;
  JA = 1;
  var R = jA, v = Symbol.for("react.element"), O = Symbol.for("react.fragment"), c = Object.prototype.hasOwnProperty, b = R.__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED.ReactCurrentOwner, y = { key: !0, ref: !0, __self: !0, __source: !0 };
  function U(G, D, q) {
    var S, x = {}, e = null, J = null;
    q !== void 0 && (e = "" + q), D.key !== void 0 && (e = "" + D.key), D.ref !== void 0 && (J = D.ref);
    for (S in D)
      c.call(D, S) && !y.hasOwnProperty(S) && (x[S] = D[S]);
    if (G && G.defaultProps)
      for (S in D = G.defaultProps, D)
        x[S] === void 0 && (x[S] = D[S]);
    return { $$typeof: v, type: G, key: e, ref: J, props: x, _owner: b.current };
  }
  return P.Fragment = O, P.jsx = U, P.jsxs = U, P;
}
var N = {};
/**
 * @license React
 * react-jsx-runtime.development.js
 *
 * Copyright (c) Facebook, Inc. and its affiliates.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */
var WA;
function Mr() {
  return WA || (WA = 1, process.env.NODE_ENV !== "production" && function() {
    var R = jA, v = Symbol.for("react.element"), O = Symbol.for("react.portal"), c = Symbol.for("react.fragment"), b = Symbol.for("react.strict_mode"), y = Symbol.for("react.profiler"), U = Symbol.for("react.provider"), G = Symbol.for("react.context"), D = Symbol.for("react.forward_ref"), q = Symbol.for("react.suspense"), S = Symbol.for("react.suspense_list"), x = Symbol.for("react.memo"), e = Symbol.for("react.lazy"), J = Symbol.for("react.offscreen"), H = Symbol.iterator, V = "@@iterator";
    function CA(A) {
      if (A === null || typeof A != "object")
        return null;
      var r = H && A[H] || A[V];
      return typeof r == "function" ? r : null;
    }
    var g = R.__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED;
    function F(A) {
      {
        for (var r = arguments.length, t = new Array(r > 1 ? r - 1 : 0), C = 1; C < r; C++)
          t[C - 1] = arguments[C];
        gA("error", A, t);
      }
    }
    function gA(A, r, t) {
      {
        var C = g.ReactDebugCurrentFrame, s = C.getStackAddendum();
        s !== "" && (r += "%s", t = t.concat([s]));
        var n = t.map(function(B) {
          return String(B);
        });
        n.unshift("Warning: " + r), Function.prototype.apply.call(console[A], console, n);
      }
    }
    var EA = !1, BA = !1, QA = !1, T = !1, o = !1, a;
    a = Symbol.for("react.module.reference");
    function l(A) {
      return !!(typeof A == "string" || typeof A == "function" || A === c || A === y || o || A === b || A === q || A === S || T || A === J || EA || BA || QA || typeof A == "object" && A !== null && (A.$$typeof === e || A.$$typeof === x || A.$$typeof === U || A.$$typeof === G || A.$$typeof === D || // This needs to include all possible module reference object
      // types supported by any Flight configuration anywhere since
      // we don't know which Flight build this will end up being used
      // with.
      A.$$typeof === a || A.getModuleId !== void 0));
    }
    function u(A, r, t) {
      var C = A.displayName;
      if (C)
        return C;
      var s = r.displayName || r.name || "";
      return s !== "" ? t + "(" + s + ")" : t;
    }
    function I(A) {
      return A.displayName || "Context";
    }
    function i(A) {
      if (A == null)
        return null;
      if (typeof A.tag == "number" && F("Received an unexpected object in getComponentNameFromType(). This is likely a bug in React. Please file an issue."), typeof A == "function")
        return A.displayName || A.name || null;
      if (typeof A == "string")
        return A;
      switch (A) {
        case c:
          return "Fragment";
        case O:
          return "Portal";
        case y:
          return "Profiler";
        case b:
          return "StrictMode";
        case q:
          return "Suspense";
        case S:
          return "SuspenseList";
      }
      if (typeof A == "object")
        switch (A.$$typeof) {
          case G:
            var r = A;
            return I(r) + ".Consumer";
          case U:
            var t = A;
            return I(t._context) + ".Provider";
          case D:
            return u(A, A.render, "ForwardRef");
          case x:
            var C = A.displayName || null;
            return C !== null ? C : i(A.type) || "Memo";
          case e: {
            var s = A, n = s._payload, B = s._init;
            try {
              return i(B(n));
            } catch {
              return null;
            }
          }
        }
      return null;
    }
    var K = Object.assign, d = 0, L, j, W, oA, DA, lA, FA;
    function fA() {
    }
    fA.__reactDisabledLog = !0;
    function mA() {
      {
        if (d === 0) {
          L = console.log, j = console.info, W = console.warn, oA = console.error, DA = console.group, lA = console.groupCollapsed, FA = console.groupEnd;
          var A = {
            configurable: !0,
            enumerable: !0,
            value: fA,
            writable: !0
          };
          Object.defineProperties(console, {
            info: A,
            log: A,
            warn: A,
            error: A,
            group: A,
            groupCollapsed: A,
            groupEnd: A
          });
        }
        d++;
      }
    }
    function zA() {
      {
        if (d--, d === 0) {
          var A = {
            configurable: !0,
            enumerable: !0,
            writable: !0
          };
          Object.defineProperties(console, {
            log: K({}, A, {
              value: L
            }),
            info: K({}, A, {
              value: j
            }),
            warn: K({}, A, {
              value: W
            }),
            error: K({}, A, {
              value: oA
            }),
            group: K({}, A, {
              value: DA
            }),
            groupCollapsed: K({}, A, {
              value: lA
            }),
            groupEnd: K({}, A, {
              value: FA
            })
          });
        }
        d < 0 && F("disabledDepth fell below zero. This is a bug in React. Please file an issue.");
      }
    }
    var sA = g.ReactCurrentDispatcher, iA;
    function _(A, r, t) {
      {
        if (iA === void 0)
          try {
            throw Error();
          } catch (s) {
            var C = s.stack.trim().match(/\n( *(at )?)/);
            iA = C && C[1] || "";
          }
        return `
` + iA + A;
      }
    }
    var aA = !1, $;
    {
      var HA = typeof WeakMap == "function" ? WeakMap : Map;
      $ = new HA();
    }
    function SA(A, r) {
      if (!A || aA)
        return "";
      {
        var t = $.get(A);
        if (t !== void 0)
          return t;
      }
      var C;
      aA = !0;
      var s = Error.prepareStackTrace;
      Error.prepareStackTrace = void 0;
      var n;
      n = sA.current, sA.current = null, mA();
      try {
        if (r) {
          var B = function() {
            throw Error();
          };
          if (Object.defineProperty(B.prototype, "props", {
            set: function() {
              throw Error();
            }
          }), typeof Reflect == "object" && Reflect.construct) {
            try {
              Reflect.construct(B, []);
            } catch (k) {
              C = k;
            }
            Reflect.construct(A, [], B);
          } else {
            try {
              B.call();
            } catch (k) {
              C = k;
            }
            A.call(B.prototype);
          }
        } else {
          try {
            throw Error();
          } catch (k) {
            C = k;
          }
          A();
        }
      } catch (k) {
        if (k && C && typeof k.stack == "string") {
          for (var E = k.stack.split(`
`), Y = C.stack.split(`
`), h = E.length - 1, M = Y.length - 1; h >= 1 && M >= 0 && E[h] !== Y[M]; )
            M--;
          for (; h >= 1 && M >= 0; h--, M--)
            if (E[h] !== Y[M]) {
              if (h !== 1 || M !== 1)
                do
                  if (h--, M--, M < 0 || E[h] !== Y[M]) {
                    var p = `
` + E[h].replace(" at new ", " at ");
                    return A.displayName && p.includes("<anonymous>") && (p = p.replace("<anonymous>", A.displayName)), typeof A == "function" && $.set(A, p), p;
                  }
                while (h >= 1 && M >= 0);
              break;
            }
        }
      } finally {
        aA = !1, sA.current = n, zA(), Error.prepareStackTrace = s;
      }
      var z = A ? A.displayName || A.name : "", bA = z ? _(z) : "";
      return typeof A == "function" && $.set(A, bA), bA;
    }
    function TA(A, r, t) {
      return SA(A, !1);
    }
    function XA(A) {
      var r = A.prototype;
      return !!(r && r.isReactComponent);
    }
    function AA(A, r, t) {
      if (A == null)
        return "";
      if (typeof A == "function")
        return SA(A, XA(A));
      if (typeof A == "string")
        return _(A);
      switch (A) {
        case q:
          return _("Suspense");
        case S:
          return _("SuspenseList");
      }
      if (typeof A == "object")
        switch (A.$$typeof) {
          case D:
            return TA(A.render);
          case x:
            return AA(A.type, r, t);
          case e: {
            var C = A, s = C._payload, n = C._init;
            try {
              return AA(n(s), r, t);
            } catch {
            }
          }
        }
      return "";
    }
    var rA = Object.prototype.hasOwnProperty, KA = {}, YA = g.ReactDebugCurrentFrame;
    function tA(A) {
      if (A) {
        var r = A._owner, t = AA(A.type, A._source, r ? r.type : null);
        YA.setExtraStackFrame(t);
      } else
        YA.setExtraStackFrame(null);
    }
    function PA(A, r, t, C, s) {
      {
        var n = Function.call.bind(rA);
        for (var B in A)
          if (n(A, B)) {
            var E = void 0;
            try {
              if (typeof A[B] != "function") {
                var Y = Error((C || "React class") + ": " + t + " type `" + B + "` is invalid; it must be a function, usually from the `prop-types` package, but received `" + typeof A[B] + "`.This often happens because of typos such as `PropTypes.function` instead of `PropTypes.func`.");
                throw Y.name = "Invariant Violation", Y;
              }
              E = A[B](r, B, C, t, null, "SECRET_DO_NOT_PASS_THIS_OR_YOU_WILL_BE_FIRED");
            } catch (h) {
              E = h;
            }
            E && !(E instanceof Error) && (tA(s), F("%s: type specification of %s `%s` is invalid; the type checker function must return `null` or an `Error` but returned a %s. You may have forgotten to pass an argument to the type checker creator (arrayOf, instanceOf, objectOf, oneOf, oneOfType, and shape all require an argument).", C || "React class", t, B, typeof E), tA(null)), E instanceof Error && !(E.message in KA) && (KA[E.message] = !0, tA(s), F("Failed %s type: %s", t, E.message), tA(null));
          }
      }
    }
    var NA = Array.isArray;
    function nA(A) {
      return NA(A);
    }
    function ZA(A) {
      {
        var r = typeof Symbol == "function" && Symbol.toStringTag, t = r && A[Symbol.toStringTag] || A.constructor.name || "Object";
        return t;
      }
    }
    function VA(A) {
      try {
        return LA(A), !1;
      } catch {
        return !0;
      }
    }
    function LA(A) {
      return "" + A;
    }
    function yA(A) {
      if (VA(A))
        return F("The provided key is an unsupported type %s. This value must be coerced to a string before before using it here.", ZA(A)), LA(A);
    }
    var X = g.ReactCurrentOwner, _A = {
      key: !0,
      ref: !0,
      __self: !0,
      __source: !0
    }, vA, xA, cA;
    cA = {};
    function $A(A) {
      if (rA.call(A, "ref")) {
        var r = Object.getOwnPropertyDescriptor(A, "ref").get;
        if (r && r.isReactWarning)
          return !1;
      }
      return A.ref !== void 0;
    }
    function Ar(A) {
      if (rA.call(A, "key")) {
        var r = Object.getOwnPropertyDescriptor(A, "key").get;
        if (r && r.isReactWarning)
          return !1;
      }
      return A.key !== void 0;
    }
    function rr(A, r) {
      if (typeof A.ref == "string" && X.current && r && X.current.stateNode !== r) {
        var t = i(X.current.type);
        cA[t] || (F('Component "%s" contains the string ref "%s". Support for string refs will be removed in a future major release. This case cannot be automatically converted to an arrow function. We ask you to manually fix this case by using useRef() or createRef() instead. Learn more about using refs safely here: https://reactjs.org/link/strict-mode-string-ref', i(X.current.type), A.ref), cA[t] = !0);
      }
    }
    function tr(A, r) {
      {
        var t = function() {
          vA || (vA = !0, F("%s: `key` is not a prop. Trying to access it will result in `undefined` being returned. If you need to access the same value within the child component, you should pass it as a different prop. (https://reactjs.org/link/special-props)", r));
        };
        t.isReactWarning = !0, Object.defineProperty(A, "key", {
          get: t,
          configurable: !0
        });
      }
    }
    function er(A, r) {
      {
        var t = function() {
          xA || (xA = !0, F("%s: `ref` is not a prop. Trying to access it will result in `undefined` being returned. If you need to access the same value within the child component, you should pass it as a different prop. (https://reactjs.org/link/special-props)", r));
        };
        t.isReactWarning = !0, Object.defineProperty(A, "ref", {
          get: t,
          configurable: !0
        });
      }
    }
    var Cr = function(A, r, t, C, s, n, B) {
      var E = {
        // This tag allows us to uniquely identify this as a React Element
        $$typeof: v,
        // Built-in properties that belong on the element
        type: A,
        key: r,
        ref: t,
        props: B,
        // Record the component responsible for creating this element.
        _owner: n
      };
      return E._store = {}, Object.defineProperty(E._store, "validated", {
        configurable: !1,
        enumerable: !1,
        writable: !0,
        value: !1
      }), Object.defineProperty(E, "_self", {
        configurable: !1,
        enumerable: !1,
        writable: !1,
        value: C
      }), Object.defineProperty(E, "_source", {
        configurable: !1,
        enumerable: !1,
        writable: !1,
        value: s
      }), Object.freeze && (Object.freeze(E.props), Object.freeze(E)), E;
    };
    function gr(A, r, t, C, s) {
      {
        var n, B = {}, E = null, Y = null;
        t !== void 0 && (yA(t), E = "" + t), Ar(r) && (yA(r.key), E = "" + r.key), $A(r) && (Y = r.ref, rr(r, s));
        for (n in r)
          rA.call(r, n) && !_A.hasOwnProperty(n) && (B[n] = r[n]);
        if (A && A.defaultProps) {
          var h = A.defaultProps;
          for (n in h)
            B[n] === void 0 && (B[n] = h[n]);
        }
        if (E || Y) {
          var M = typeof A == "function" ? A.displayName || A.name || "Unknown" : A;
          E && tr(B, M), Y && er(B, M);
        }
        return Cr(A, E, Y, s, C, X.current, B);
      }
    }
    var IA = g.ReactCurrentOwner, pA = g.ReactDebugCurrentFrame;
    function m(A) {
      if (A) {
        var r = A._owner, t = AA(A.type, A._source, r ? r.type : null);
        pA.setExtraStackFrame(t);
      } else
        pA.setExtraStackFrame(null);
    }
    var hA;
    hA = !1;
    function wA(A) {
      return typeof A == "object" && A !== null && A.$$typeof === v;
    }
    function RA() {
      {
        if (IA.current) {
          var A = i(IA.current.type);
          if (A)
            return `

Check the render method of \`` + A + "`.";
        }
        return "";
      }
    }
    function Er(A) {
      {
        if (A !== void 0) {
          var r = A.fileName.replace(/^.*[\\\/]/, ""), t = A.lineNumber;
          return `

Check your code at ` + r + ":" + t + ".";
        }
        return "";
      }
    }
    var OA = {};
    function Br(A) {
      {
        var r = RA();
        if (!r) {
          var t = typeof A == "string" ? A : A.displayName || A.name;
          t && (r = `

Check the top-level render call using <` + t + ">.");
        }
        return r;
      }
    }
    function GA(A, r) {
      {
        if (!A._store || A._store.validated || A.key != null)
          return;
        A._store.validated = !0;
        var t = Br(r);
        if (OA[t])
          return;
        OA[t] = !0;
        var C = "";
        A && A._owner && A._owner !== IA.current && (C = " It was passed a child from " + i(A._owner.type) + "."), m(A), F('Each child in a list should have a unique "key" prop.%s%s See https://reactjs.org/link/warning-keys for more information.', t, C), m(null);
      }
    }
    function dA(A, r) {
      {
        if (typeof A != "object")
          return;
        if (nA(A))
          for (var t = 0; t < A.length; t++) {
            var C = A[t];
            wA(C) && GA(C, r);
          }
        else if (wA(A))
          A._store && (A._store.validated = !0);
        else if (A) {
          var s = CA(A);
          if (typeof s == "function" && s !== A.entries)
            for (var n = s.call(A), B; !(B = n.next()).done; )
              wA(B.value) && GA(B.value, r);
        }
      }
    }
    function Qr(A) {
      {
        var r = A.type;
        if (r == null || typeof r == "string")
          return;
        var t;
        if (typeof r == "function")
          t = r.propTypes;
        else if (typeof r == "object" && (r.$$typeof === D || // Note: Memo only checks outer props here.
        // Inner props are checked in the reconciler.
        r.$$typeof === x))
          t = r.propTypes;
        else
          return;
        if (t) {
          var C = i(r);
          PA(t, A.props, "prop", C, A);
        } else if (r.PropTypes !== void 0 && !hA) {
          hA = !0;
          var s = i(r);
          F("Component %s declared `PropTypes` instead of `propTypes`. Did you misspell the property assignment?", s || "Unknown");
        }
        typeof r.getDefaultProps == "function" && !r.getDefaultProps.isReactClassApproved && F("getDefaultProps is only used on classic React.createClass definitions. Use a static property named `defaultProps` instead.");
      }
    }
    function or(A) {
      {
        for (var r = Object.keys(A.props), t = 0; t < r.length; t++) {
          var C = r[t];
          if (C !== "children" && C !== "key") {
            m(A), F("Invalid prop `%s` supplied to `React.Fragment`. React.Fragment can only have `key` and `children` props.", C), m(null);
            break;
          }
        }
        A.ref !== null && (m(A), F("Invalid attribute `ref` supplied to `React.Fragment`."), m(null));
      }
    }
    function kA(A, r, t, C, s, n) {
      {
        var B = l(A);
        if (!B) {
          var E = "";
          (A === void 0 || typeof A == "object" && A !== null && Object.keys(A).length === 0) && (E += " You likely forgot to export your component from the file it's defined in, or you might have mixed up default and named imports.");
          var Y = Er(s);
          Y ? E += Y : E += RA();
          var h;
          A === null ? h = "null" : nA(A) ? h = "array" : A !== void 0 && A.$$typeof === v ? (h = "<" + (i(A.type) || "Unknown") + " />", E = " Did you accidentally export a JSX literal instead of a component?") : h = typeof A, F("React.jsx: type is invalid -- expected a string (for built-in components) or a class/function (for composite components) but got: %s.%s", h, E);
        }
        var M = gr(A, r, t, s, n);
        if (M == null)
          return M;
        if (B) {
          var p = r.children;
          if (p !== void 0)
            if (C)
              if (nA(p)) {
                for (var z = 0; z < p.length; z++)
                  dA(p[z], A);
                Object.freeze && Object.freeze(p);
              } else
                F("React.jsx: Static children should always be an array. You are likely explicitly calling React.jsxs or React.jsxDEV. Use the Babel transform instead.");
            else
              dA(p, A);
        }
        return A === c ? or(M) : Qr(M), M;
      }
    }
    function sr(A, r, t) {
      return kA(A, r, t, !0);
    }
    function ir(A, r, t) {
      return kA(A, r, t, !1);
    }
    var ar = ir, nr = sr;
    N.Fragment = c, N.jsx = ar, N.jsxs = nr;
  }()), N;
}
(function(R) {
  process.env.NODE_ENV === "production" ? R.exports = ur() : R.exports = Mr();
})(wr);
const uA = eA.jsx, Dr = eA.jsxs;
const lr = 1e3, Fr = 800;
let w = [], MA = 1, f = {}, Q = {}, Z = -1;
const Sr = cr(
  ({
    url: R,
    lineProps: v,
    containerProps: O,
    pointProps: c,
    getPoints: b,
    fullScreen: y = !1
  }, U) => {
    const [G, D] = UA(!1), [q, S] = UA(!1), x = qA(null), e = qA(null);
    Ir(() => {
      var o, a, l;
      return (o = e.current) == null || o.addEventListener("wheel", V), (a = e.current) == null || a.addEventListener("mousedown", J), (l = e.current) == null || l.addEventListener("mousemove", H), () => {
        var u, I, i;
        f.clearRect(0, 0, 1e5, 1e5), w = [], (u = e.current) == null || u.removeEventListener("wheel", V), (I = e.current) == null || I.removeEventListener("mousedown", J), (i = e.current) == null || i.removeEventListener("mousemove", H);
      };
    }, []);
    const J = (o) => {
      if (!e.current)
        return;
      const { clientX: a, clientY: l, offsetX: u, offsetY: I } = o, i = a - e.current.offsetLeft || 0, K = l - e.current.offsetTop || 0, d = c != null && c.size ? (c == null ? void 0 : c.size) * 2 : 20;
      w.forEach((L, j) => {
        const W = { x: L.x * g(), y: L.y * g() };
        Math.abs(u - W.x) <= d && Math.abs(I - W.y) <= d && (Z = j);
      }), Z >= 0 ? e.current.onmousemove = function(L) {
        w[Z].x = L.offsetX / g(), w[Z].y = L.offsetY / g(), f.clearRect(
          0,
          0,
          Q.clientWidth * g(),
          Q.clientHeight * g()
        ), f.drawImage(
          Q,
          0,
          0,
          Q.clientWidth * g(),
          Q.clientHeight * g()
        ), T(g());
      } : document.onmousemove = function(L) {
        if (!e.current)
          return;
        const j = L.clientX - i, W = L.clientY - K;
        e.current.style.left = j + "px", e.current.style.top = W + "px";
      }, document.onmouseup = function() {
        var L;
        e.current && ((L = e.current) == null || L.setAttribute("class", ""), document.onmousemove = null, e.current.onmousemove = null, document.onmouseup = null, Z = -1);
      };
    }, H = (o) => {
      if (!e.current)
        return;
      const { offsetX: a, offsetY: l } = o;
      let u = !1;
      const I = c != null && c.size ? (c == null ? void 0 : c.size) * 2 : 20;
      w.forEach((i) => {
        const K = { x: i.x * g(), y: i.y * g() };
        Math.abs(a - K.x) <= I && Math.abs(l - K.y) <= I && (u = !0);
      }), setTimeout(() => {
        var i;
        (i = e.current) == null || i.setAttribute("class", u ? "selected" : "");
      }, 100);
    }, V = (o) => {
      o.preventDefault(), CA(o), T(g());
    }, CA = (o) => {
      var K;
      let a = 0, l = 0;
      const { deltaY: u, offsetX: I, offsetY: i } = o;
      u < 0 && g() < 5 && (e != null && e.current) && (Q.style.scale = JSON.stringify(g() + 0.1), l = ((K = e.current) == null ? void 0 : K.offsetLeft) - (I * g() / (g() - 0.1) - I), a = e.current.offsetTop - (i * g() / (g() - 0.1) - i), e.current.style.left = l + "px", e.current.style.top = a + "px", e.current.style.width = Q.clientWidth * g() + "px", e.current.style.height = Q.clientHeight * g() + "px", e.current.width = Q.clientWidth * g(), e.current.height = Q.clientHeight * g()), u >= 0 && g() - 0.1 >= 0.1 && (e != null && e.current) && (Q.style.scale = JSON.stringify(g() - 0.1), l = e.current.offsetLeft - I * g() / (g() + 0.1) + I, a = e.current.offsetTop - i * g() / (g() + 0.1) + i, e.current.style.left = l + "px", e.current.style.top = a + "px", e.current.style.width = Q.clientWidth * g() + "px", e.current.style.height = Q.clientHeight * g() + "px", e.current.width = Q.clientWidth * g(), e.current.height = Q.clientHeight * g()), f.drawImage(
        Q,
        0,
        0,
        Q.clientWidth * g(),
        Q.clientHeight * g()
      );
    }, g = () => Number(Q.style.scale), F = () => {
      var o;
      f = (o = e == null ? void 0 : e.current) == null ? void 0 : o.getContext("2d"), D(!0), Q = document.createElement("img"), Q.setAttribute("alt", "trace img"), Q.setAttribute("id", "img"), Q.setAttribute("src", R), Q.onload = function() {
        var i;
        D(!1), (i = x.current) == null || i.append(Q);
        const { width: a, height: l } = Q;
        a > l ? Q.style.width = "80%" : Q.style.height = "80%";
        const { clientWidth: u, clientHeight: I } = Q;
        MA = u / a, e.current && (e.current.style.width = u + "px", e.current.style.height = I + "px", e.current.width = u, e.current.height = I, e.current.style.left = `calc(50% - ${u / 2}px)`, e.current.style.top = `calc(50% - ${I / 2}px)`), f.drawImage(Q, 0, 0, u, I), Q.style.scale = "1", EA(), T(1);
      }, Q.onerror = function() {
        D(!1), S(!0);
      };
    }, gA = () => {
      w = [], F();
    }, EA = () => {
      if (w.length > 0)
        return;
      const { clientWidth: o, clientHeight: a } = Q, l = {
        x: o * (y ? 0 : 0.2),
        y: a * (y ? 0 : 0.2)
      }, u = {
        x: o * (y ? 1 : 0.8),
        y: a * (y ? 0 : 0.2)
      }, I = {
        x: o * (y ? 1 : 0.8),
        y: a * (y ? 1 : 0.8)
      }, i = {
        x: o * (y ? 0 : 0.2),
        y: a * (y ? 1 : 0.8)
      };
      w.push(l), w.push(u), w.push(I), w.push(i);
    }, BA = () => w.length ? {
      points: w,
      originPoints: w.map(({ x: o, y: a }) => ({
        x: Math.floor(Number(o / MA)),
        y: Math.floor(Number(a / MA))
      }))
    } : (console.error("画布暂时没有初始化"), {
      points: w,
      originPoints: []
    }), QA = (o = []) => {
      w = o, F();
    }, T = (o) => {
      w.length && (w.forEach((a) => {
        f.beginPath(), f.arc(
          a.x * o,
          a.y * o,
          (c == null ? void 0 : c.size) || 10,
          0,
          2 * Math.PI
        ), f.fillStyle = (c == null ? void 0 : c.color) || "rgba(255,0,0,0.5)", f.fill(), f.closePath();
      }), f.beginPath(), f.lineWidth = (v == null ? void 0 : v.width) || 2, f.strokeStyle = (v == null ? void 0 : v.color) || "red", w.forEach(({ x: a, y: l }) => {
        f.lineTo(a * o, l * o);
      }), f.lineTo(w[0].x * o, w[0].y * o), f.stroke(), b && b(BA()));
    };
    return hr(U, () => ({
      onReset: gA,
      setPoints: QA
    })), /* @__PURE__ */ Dr(
      "div",
      {
        ref: x,
        className: "label-image__container",
        style: {
          width: `${(O == null ? void 0 : O.width) || lr}px`,
          height: `${(O == null ? void 0 : O.height) || Fr}px`
        },
        children: [
          G && /* @__PURE__ */ uA("div", { className: "status", children: /* @__PURE__ */ uA(
            "img",
            {
              className: "image-label_loading",
              width: 100,
              height: 100,
              alt: "loading icon",
              src: "data:image/gif;base64,"
            }
          ) }),
          /* @__PURE__ */ uA("canvas", { ref: e })
        ]
      }
    );
  }
);
export {
  Sr as default
};
